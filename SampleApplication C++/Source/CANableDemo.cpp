...
void  DfuDemo();
DWORD OpenDevice(Candlelight& i_Candle, BYTE u8_Interface);
...
void CandlelightDemo()
{
    ...
    Candlelight  i_Candle;
    DWORD u32_Error = OpenDevice(i_Candle, 0);
    ...
    {
        PrintConsole(GREY, L"USB Endpoint IN:      %02X,  max packet size: %u\n", i_Candle.mu8_EndpointIN, i_Candle.mu16_MaxPackSizeIN);
        PrintConsole(GREY, L"USB Endpoint OUT:     %02X,  max packet size: %u\n", i_Candle.mu8_EndpointOUT,i_Candle.mu16_MaxPackSizeOUT);

        // If the processor clock is available -> display it
        if (u32_Error && i_Candle.mk_Capability.fclk_can > 0)
            PrintConsole(GREY, L"CAN Clock:            %u MHz\n", i_Candle.mk_Capability.fclk_can / 1000000);

        PrintConsole(GREY, L"Hardware Version:     %s\n", i_Candle.FormatBcdVersion(i_Candle.mk_DeviceVersion.hw_version_bcd));          
        PrintConsole(GREY, L"Firmware Version:     %s\n", i_Candle.FormatBcdVersion(i_Candle.mk_DeviceVersion.sw_version_bcd));
        PrintConsole(GREY, L"Firmware Type:        %s\n", i_Candle.mb_LegacyFirmware ? L"Legacy" : L"CANable 2.5");
        PrintConsole(GREY, L"Supports CAN FD:      %s\n", i_Candle.mb_SupportsFD     ? L"Yes"    : L"No");
    }

    if (u32_Error)
    {
        PrintConsole(RED, L"\n%s\n", i_Candle.FormatLastError(u32_Error));
        return;
    }

    // The board info is only available with the new CANable 2.5 firmware
    PrintConsole(GREY, L"Target Board:         %hs\n", i_Candle.mk_BoardInfo.BoardName);
    PrintConsole(GREY, L"Processor:            %hs, CAN Clock: %u MHz, DeviceID: 0x%X\n",
                                                       i_Candle.mk_BoardInfo.McuName,
                                                       i_Candle.mk_Capability.fclk_can / 1000000,
                                                       i_Candle.mk_BoardInfo.McuDeviceID);

    // -----------------------------------------

    CString s_Display;
    // Supposed the clock is 160 MHz this will set 500 kBaud and samplepoint 87.5%
    u32_Error = i_Candle.SetBitrate(false, 2, 139, 20, &s_Display);
    if (u32_Error)
    {
    ...
    u32_Error = i_Candle.EnableBusLoadReport(5);
    if (u32_Error)
    ...
    if (true)
    {
        ...
        u32_Error = i_Candle.SetBitrate(true, 2, 29, 10, &s_Display);
        if (u32_Error)
        {
    ...
    if (false)
    {
        ...
        u32_Error = i_Candle.AddMaskFilter(false, 0x7E8, 0x7FF);
        if (u32_Error)
    ...
    u32_Error = i_Candle.Start((eDeviceFlags)u32_DevFlags);
    if (u32_Error)
    {
    ...
    bool b_BootPin;
    u32_Error = i_Candle.IsBootPinEnabled(&b_BootPin);
    if (u32_Error)
        PrintConsole(RED,  L"Error %u getting status of pin BOOT0\n", u32_Error);
    else
    {
        PrintConsole(GREY,  i_Candle.FormatTimestamp(NULL, i_Candle.GetWinTimestamp()));
    ...
    {
        // Read the comment of GetWinTimestamp()
        __int64 s64_Now = i_Candle.GetWinTimestamp();

        // Send the Tx frame every 2 seconds (= 200000 * 10 µs)
        if (s64_Now - s64_LastStamp >= 200000)
        {
            s64_LastStamp = s64_Now;

            __int64 s64_TxStamp; // only valid if no error returned
            CString   s_TxPacket;
            u32_Error = i_Candle.SendPacket(u32_TxID, false, false, false, false, u8_TxData, sizeof(u8_TxData), &s_TxPacket, &s64_TxStamp);
            if (u32_Error)
            {
                PrintConsole(GREY,  i_Candle.FormatTimestamp(NULL, i_Candle.GetWinTimestamp()));
            ...
            else
            {
                ...
                PrintConsole(GREY,  i_Candle.FormatTimestamp(NULL, s64_TxStamp));
                PrintConsole(WHITE, L" Send");
        ...
        u32_Error = i_Candle.ReceiveData(100, pk_Header, sizeof(u8_RxData), &s64_WinTimestamp);
        if (u32_Error)
        {
            ...
            if (u32_Error != ERROR_TIMEOUT)
            {
                ...
                PrintConsole(GREY,  i_Candle.FormatTimestamp(NULL, s64_WinTimestamp));
        ...
        {
            PrintConsole(GREY, i_Candle.FormatTimestamp(pk_Header, s64_WinTimestamp));
            switch (pk_Header->msg_type)
            {
                case MSG_RxFrame:
                {
                    CString s_RxPacket = i_Candle.FormatRxPacket((kRxFrameElmue*)pk_Header);
                    PrintConsole(WHITE, L" Recv");
                ...
                case MSG_TxEcho:
                {
                    CString s_EchoPacket = i_Candle.FormatTxEcho((kTxEchoElmue*)pk_Header);
                ...
                case MSG_Error:
                {
                    ...
                    CString s_Error = i_Candle.FormatCanErrors((kErrorElmue*)pk_Header, &e_BusStatus, &e_ErrLevel);
                    WORD u16_Color = GREY;
...
void DfuDemo()
{
    ...
    Candlelight i_Candle;
    DWORD u32_Error = OpenDevice(i_Candle, 1);
    if (u32_Error)
    {
        PrintConsole(RED, L"%s\n", i_Candle.FormatLastError(u32_Error));
        return;
    ...
    u32_Error = i_Candle.EnterDfuMode();
    if (u32_Error)
    {
        PrintConsole(RED, L"%s\n", i_Candle.FormatLastError(u32_Error));
        return;
...
DWORD OpenDevice(Candlelight& i_Candle, BYTE u8_Interface)
{
    ...
    DWORD u32_Error = i_Candle.EnumDevices(u8_Interface, &i_DispNames, &i_DevicePaths);
    if (u32_Error)
    {
        PrintConsole(RED, L"%s\n", i_Candle.FormatLastError(u32_Error));
        return u32_Error;
    ...
    u32_Error = i_Candle.Open(i_DevicePaths[s32_DeviceIndex]);
    ...
    if (u32_Error == 0 || u32_Error == ERROR_INVALID_DEVICE || u32_Error == ERROR_INVALID_FIRMWARE)
    {
        ...
        PrintConsole(GREY, L"USB Vendor  ID:       %04X\n", i_Candle.mk_DeviceDescr.idVendor);
        PrintConsole(GREY, L"USB Product ID:       %04X\n", i_Candle.mk_DeviceDescr.idProduct);
        PrintConsole(GREY, L"USB Device Version:   %s\n",   i_Candle.FormatBcdVersion(i_Candle.mk_DeviceDescr.bcdDevice));
    }
    return u32_Error;
}
