...
// This class implements the new CANable 2.5 Elm�Soft protocol.
...
uint32_t Candlelight::Open(kUsbDevice* pk_Device)
{
    ...
    if (!mpk_Info->mb_IsElmueSoft)
    {
        ...
        return ERR_INVALID_FIRMWARE; // this class requires the new Elm�Soft firmware
    }

    // --------------- Here comes only Elm�Soft firmware ---------------

    // ELM_ReqGetBoardInfo requires Elm�Soft firmware
...
// This function can obtain the feedback from the Elm�Soft firmware, but works also with legacy firmware.
...
// Formats a timestamp with 1 �s precision
...
string Candlelight::FormatLastError(uint32_t u32_Error)
{
    ...
    switch (u32_Error)
    {
        ...
        case ERR_INVALID_FIRMWARE:  return "This demo supports only devices that have the CANable 2.5 firmware from Elm�Soft.";
...
// IMPORTANT:
// This will ONLY work if the Candlelight has the new CANable 2.5 firmware from Elm�Soft.
...
uint32_t Candlelight::EnterDfuMode()
{
    ...
    if (CtrlTransfer(DIR_In, DFU_RequGetStatus, 0, &k_Status, sizeof(k_Status)) == NO_ERROR)
    {
        ...
        // returning AppDetach has been added by Elm�Soft to the firmware and means that the user must reconnect the USB cable.
