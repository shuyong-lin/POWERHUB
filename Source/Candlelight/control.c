...
{
    // GetLastError always sends a valid response even if the Elm�Soft protocol is not enabled or the host sends an invalid channel.
    ...
    // To enable the new Elm�Soft commands the host must send as the first command:
    ...
    else  // -------- IN: Device -> Host (error checking here) --------
    {
        ...
        switch (req->bRequest)
        {
            ...
            case GS_ReqGetTimestamp:
                ...
                // This is totally stupid, because the timestamp has a precision of 1 �s, but SOF packets are received once every millisecond.
...
// So the ONLY way to transmit errors of SETUP requests to the host is with the new Elm�Soft protocol and command ELM_ReqGetLastError.
    ...
    switch (last_setup.bRequest)
    {
        ...
        case GS_ReqSetDeviceMode:
        {
            ...
            if (GLB_ProtoElmue)
            {
                ...
                // When the Elm�Soft protocol is enabled, also debug messages and error reports are enabled by default.
                for (int C=0; C<CHANNEL_COUNT; C++)
                {
...
{
    // only called for Elm�Soft protocol
...
// Only the new Elm�Soft firmware allows to set flags when closing the device.
