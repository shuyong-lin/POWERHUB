...
float Vbus = 0.0f; // Global variable to store the ADC value
/* USER CODE BEGIN 1 */
void adc_timer_100ms(uint32_t tick_now)
{
  HAL_ADC_Start(&hadc1);
  if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK)
  {
    uint32_t adc_value = HAL_ADC_GetValue(&hadc1);
    Vbus = adc_value * 3.3f / 4095.0f * 17.9f; // assuming 12-bit ADC and 3.3V reference
